...
S3 (CloudTrail 로그)
            ↓
  Lambda (하루 3회: KST 02:00 / 10:00 / 18:00, EventBridge)
...
### 목적
AWS 인프라 자체가 장애를 겪는 상황에 대비해, CloudTrail 보안 감사 로그를
Azure에 이중화하여 장기 보존 및 재해복구용으로 조회할 수 있도록 합니다.

> **역할 분리**: 실시간 침해 대응(로그인/삭제 즉시 알림)은 CloudTrail → Teams 알림이 담당하고,
> Azure 백업은 AWS 장애 시 사후 감사·재해복구용으로 역할을 분리했습니다.
> 동기화 주기는 하루 1회에서 **하루 3회로 개선**하여 장애 시점과 백업 시점의 간격을 최소화했습니다.
...
| 실행 주기 | 하루 3회 KST 02:00 / 10:00 / 18:00 (EventBridge cron) |
...
Log Analytics Workspace (siseon-security-logs)
  ...
  • 보존: 30일
        ↓
Azure Monitor Workbook
  (보안 중심 재설계 — 조회성 노이즈 제외, 변경·인증 이벤트 중심)
...
### KQL 쿼리 예시
```kusto
// 변경 작업 Top (조회성 Describe/Get/List 제외 — 보안 의미 있는 이벤트만)
CloudTrailLogs_CL
| where TimeGenerated > ago(24h)
| where EventName_s startswith "Delete" or EventName_s startswith "Create"
     or EventName_s startswith "Terminate" or EventName_s startswith "Modify"
     or EventName_s startswith "Put" or EventName_s startswith "Update"
